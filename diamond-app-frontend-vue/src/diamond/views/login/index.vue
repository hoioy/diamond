<template>
  <div class="login-container">
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <el-row>
        <el-col>
          <el-button :loading="loading" type="warning" round style="width:100%;margin-bottom:30px;" @click.native.prevent="handleLoginByDiamondOS">
            Diamond统一认证登录
          </el-button>
        </el-col>
      </el-row>
...
import sm4 from '@/utils/sm4/SM4Encode'
export default {
  name: 'Login',
  data() {
    ...
    return {
      ...
      loginRules: {
        ...
        captchaCode: [{ required: true, trigger: 'change', message: '请填写验证码' }, { validator: remoteValidator, trigger: 'change', message: '验证码错误' }]
      },
      remoteError: null,
      captchaBase64: null,
  ...
  methods: {
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true
          if (process.env.VUE_APP_SM4_ENABLE === 'true') {
            const _pwd = sm4.encrypt_ecb(this.loginForm.password, process.env.VUE_APP_SM4_KEY)
            this.loginForm.password = _pwd
          }
