...
export function param2ObjForSplit(url) {
  const search = url.split('/')[url.split('/').length - 1]
  if (!search) {
    return {}
  }
  return JSON.parse(
...
export function fieldQuery(array, query) {
  const keys = Object.keys(query)
  return array.filter(item => {
    return keys.every(key => {
      return true
...
// 换肤加class函数
export function toggleClass2(element, className) {
  if (!element || !className) {
    return
  }
