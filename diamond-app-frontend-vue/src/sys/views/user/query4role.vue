    ...
    <el-card>
      <el-col :span="24">
        <el-form :model="queryCriteria" label-width="150px">
          ...
          <!--<el-input v-model="queryCriteria.roleId" placeholder="请输入roleId"/>-->
          <!--</el-form-item>-->
          <!--</el-col>-->
          <el-col :span="24">
            <flex-center>
  ...
  methods: {
    ...
    executeQueryPage() {
      if (this.datasf2c4submit === 'submitSuccess') {
        ...
        // console.info('--------------------------------------' + this.datasf2c4submit)
      }
      // console.info('执行一次---')
      // console.info(this.checkedAddMenusForCurrentPage)
      // console.info(this.checkedDelMenusForCurrentPage)
      ...
      UserAPI.queryPageUsersByRole(this.createQueryParams()).then(data => {
        ...
        // console.info(this.roleByUserIds)
        ...
        if (this.datasf2c4dpetrole === 'dept') {
          UserAPI.findDeptByUserIds((this.roleByUserIds)).then(data => { // TODO
            ...
            data.forEach(row => {
              ...
              // if (this.checkedAddMenus.indexOf(row.userId) >= 0) { // TODO
              //   // 已经勾选过的也需要展示出来
              //   this.checkedMenusForPage.push(row.userId)
              // }
            })
            ...
            // console.info('this.checkedMenusForPage---')
            // console.info(this.checkedMenusForPage)
            // console.info('this.checkedMenusForPage---')
          }).then(() => {
    ...
    clickCheckboxHandler(selection) {
      ...
      // console.info('this.checkedMenuIds')
      // console.info(this.checkedMenuIds)
      // console.info('this.checkedMenuIds')
      ...
      // 如果勾选后又删除，则按照这个逻辑删除
      console.info('---------------checkedAddMenusForCurrentPage--------------')
      console.info(this.checkedAddMenusForCurrentPage)
      console.info('---------------checkedAddMenusForCurrentPage--------------')
      // 如果用户又将已经新勾选的选项删除，则按照下面逻辑删除，判断临时的新增集合中是否存在，对比当前页勾选的情况，
      // 如果临时集合中有，当前页又取消了，则删除临时缓存和最终提交的缓存
      // this.checkedAddMenusForCurrentPage.forEach(row => {
      ...
      // console.info(this.checkedMenus)
      // console.info('---------------checkedMenus--------------')
      // console.info('---------------checkedMenuIds--------------')
      // console.info(this.checkedMenuIds)
      // console.info('---------------checkedMenuIds--------------')
      ...
      // 去重--------------
      // console.info('---------------checkedDelMenusForCurrentPage--------------')
      // console.info(this.checkedDelMenusForCurrentPage)
      // console.info('---------------checkedDelMenusForCurrentPage--------------')
      // 如果本页临时缓存中在本页勾选的集合中也发现了相同元素，则说明该元素在取消勾选后，又被重新勾选，需要从最终提交删除集合中去掉
      // this.checkedDelMenusForCurrentPage.forEach(row => {
      ...
      console.info(this.checkedAddMenus)
      console.info(this.checkedDelMenus)
