  ...
  <el-row>
      ...
      <button-right>
        ...
        <template slot="button">
          <el-button-group>
            ...
            <!--            <el-button v-if="hasPerm('dictionary-type/add') || hasPerm('dictionary-type/*')" type="primary" @click="$emit('option-changed','add')">-->
    ...
    <el-col :span="24">
      <el-table :data="pagination.list" highlight-current-row stripe border @current-change="changeSelected" @row-dblclick="$emit('option-changed','check', selected)" @sort-change="sortChangeHandler">
        <el-table-column :show-overflow-tooltip="true" prop="typeName" label="名称" sortable="custom"/>
