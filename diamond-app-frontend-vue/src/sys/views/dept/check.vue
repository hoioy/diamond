    ...
    <el-collapse value="base-info" accordion>
      <el-collapse-item title="基本信息" name="base-info">
        <el-form :model="detail" :label-width="labelWidth">
          ...
          <input-item-view v-if="parentDeptName" label="上级部门">
            {{ parentDeptName }}
          </input-item-view>
          <!--<input-item-view label="启用状态">-->
            <!--{{ detail.state | translateState }}-->
          <!--</input-item-view>-->
          <input-item-view label="部门排序号">
            {{ detail.index }}
          </input-item-view>
          <input-item-view label="部门类型">
            {{ detail.type }}
          </input-item-view>
          <input-item-view label="部门名称">
            {{ detail.deptName }}
          </input-item-view>
          <text-item-view label="部门备注">
            {{ detail.remark }}
          </text-item-view>
        </el-form>
      </el-collapse-item>
