  ...
  <div>
    ...
    <el-card>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="200px">
        ...
        <el-form-item label="分组排序号" prop="groupIndex">
          <el-input v-model.number="form.groupIndex" type="text"/>
        </el-form-item>
    ...
    <el-card header="用户信息">
      <el-table :data="users" border style="width: 100%">
        ...
        <el-table-column label="操作" width="100" align="center">
          <template slot-scope="scope">
            <el-button type="warning" @click="delGroupUserHandler(scope.row.id)">
              删除
            </el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  activated() {
    ...
    this.queryAllUsers()
    // this.queryAllGroupRoles(this.detail.id)
    const params = {
      filters: [],
      sorts: []
    }
  ...
  methods: {
    ...
    //     roleId: id
    //   })
    //   GroupAPI.addGroupRole(groupRoles).then(data => {
    //     this.queryAllGroupRoles(this.detail.id)
    //     this.optionSuccessHandler()
