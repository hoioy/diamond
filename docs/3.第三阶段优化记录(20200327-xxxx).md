...
1. Domain和DTO原子化,所有Domain和DTO保持简洁，原因:
    ...
    1. 为rest接口原子化做准备
    1. 可以删除各种xxxMixin类
1. diamond-security不依赖tdf-sys，抽取TDFUserDTO到common中，从而统一TDF平台的用户.与sys拆分更加清晰
...
sys模块没有前后端是否分离的概念，sys模块专注在业务逻辑，其中tdf-sys-web的不分离版本只控制导航逻辑。sys模块与security模块完全拆分,互不依赖：
    1. 前后端分离概念清晰化。
    1. diamond-sys-web不依赖tdf-security，但是依赖spring security。
...
1. log模块切入点表达式改为：within(com.hoioy.diamond.common.api.BaseController+  从而可以定义日志规范。如果默认规则不满足需求，在tdf-log-sample中将提供动态
...
#### 架构不合理优化
1. filter层的统一异常
1. diamond log已经抽取了IWebLogService,需要提供一个用户使用tdf log模块的sample，自定义切面使用Log.
...
1. diamond-common中不应该有@Component类似的注解，防止用户单独依赖tdf-common时候自动注入不必要的对象而引起冲突问题。
...
#### diamond-sample-jwt
1. 考虑tdf-sample-jwt是否有必要拆分为jpa和mybatis两个。
1. 增加单独使用tdf-common、diamond-sys、单独使用tdf-security、单独使用tdf-log的sample示例。
