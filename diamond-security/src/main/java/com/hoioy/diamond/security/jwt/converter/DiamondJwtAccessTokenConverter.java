 ...
 */
public class DiamondJwtAccessTokenConverter extends AbstractCustomJwtAccessTokenConverter {
    ...
    public DiamondJwtAccessTokenConverter() {
    }

    public DiamondJwtAccessTokenConverter(String secret, Long expiration, Long refresh) {
        this.secret = secret;
    ...
    public JwtClaimDTO convert(String token) throws IOException {
        token = DiamondJwtDesUtil.decrypt(token, secret);
    ...
    private String doGenerateToken(JwtClaimDTO payload) throws JsonProcessingException {
        ...
        return DiamondJwtDesUtil.encrypt(jwt.getEncoded(), secret);
    }
