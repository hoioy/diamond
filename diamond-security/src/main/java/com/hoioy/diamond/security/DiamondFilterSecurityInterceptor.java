 ...
 */
public class DiamondFilterSecurityInterceptor extends AbstractSecurityInterceptor
        implements Filter {
    //与security-config.xml里的myFilter的属性securityMetadataSource对应，
    // 其他的两个组件，已经在AbstractSecurityInterceptor定义
    private DiamondSecurityMetadataSource fisMetadataSource;


    public DiamondSecurityMetadataSource getFisMetadataSource() {
...
//	this.getAccessDecisionManager().decide(authenticated, fi, attributes);
        ...
        if (authenticated != null) {
            if (authenticated.getPrincipal() != null) {
                if (authenticated.getPrincipal() instanceof DiamondUser) {
                    DiamondUser user = ((DiamondUser) authenticated.getPrincipal());
                    DiamondUserDTO userInfoDto = user.getUserInfoDto();//fisMetadataSource.findUserDtoByLoginName(user.getUsername());
                    if (userInfoDto != null) {
    ...
    public void setFisMetadataSource(DiamondSecurityMetadataSource fisMetadataSource) {
