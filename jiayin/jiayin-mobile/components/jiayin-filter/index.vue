		...
		<view class="jiayin-filter" :style="{'position': fixed?'fixed':'relative','top':top}"  id="jiayin-filter-header">
			<view class="items" :class="activeIndex==index?'text-red':'text-grey'" v-for="(item, index) in filters" :key="index"
			 :data-index="index" @tap="changeSort">
				...
				<text v-else>{{ activeOption && activeOption.name || item.title }}</text>
				...
				<image src="/static/img/jiayin-filter/sort0.png" mode="widthFix" v-else-if="activeIndex!=index"></image>
				<image src="/static/img/jiayin-filter/sort2.png" mode="widthFix" v-else-if="activeIndex==index && activeAscState"></image>
				<image src="/static/img/jiayin-filter/sort1.png" mode="widthFix" v-else-if="activeIndex==index"></image>
			</view>
			...
			<!-- 下拉选项 -->
			<view class='jiayin-filter-options' v-if="activeIndex == index && showOption" v-for="(item, index) in filters" :key="index+10000">
				<view :class="[activeOption && (opt.value ===  activeOption.value) ? 'option current' : 'option']" :data-index="index"
				 :data-optindex="optIndex" v-for="(opt, optIndex) in item.options||[]" :key="optIndex+100000" @tap="changeSort">
